...
- 라이트/다크 모드(헤더 🌙, `html[data-theme=light]`), 모바일 반응형(`@media≤640px` 세로 스택), 패널 접기(좌/우), 실 DB·AI·단수카운터 제거.

## 다음 작업 (우선순위)
1. ★**프로크리에이트식 UI 전면개편** — 전체화면 캔버스 + 플로팅 툴바, 모바일 우선, "누가 써도 편하게". (사용자 최우선 요청)
2. **선택/영역 버튼 미동작**(onDown 분기 없음) → 선택영역 변형(이동·회전) 툴.
3. **커스텀 기호 편집기** + 표준 코바늘·대바늘 기호 전세트(직접 자작). 여러 기호 조합, 동일 크기 정규화. (Pro)
