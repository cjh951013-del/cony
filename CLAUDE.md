...
## 다음 작업 (우선순위)
1. **방안뜨기 전용모드** + 부채꼴/방사 배치(앵커 중심), 다중 프레임(Pro).
2. **캔버스 코·단 번호매기기**(좌하단=1→우·상), 단수표 자동, 범례 자동, 주석 레이어.
3. 표준 기호 세트 확장(앞/뒤걸어뜨기·클러스터·보블·케이블 등), 색상모델 정리(#14).
4. (완료) ~~커스텀 기호 편집기~~ / ~~프로크리에이트 UI 전면개편~~ / ~~선택·영역 변형 툴~~.
4. **방안뜨기 전용모드**, 부채꼴/방사 배치(앵커 중심), 다중 프레임(Pro).
